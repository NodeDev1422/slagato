jQuery(document).ready(function($){
	...
	if($('#form-contacts-ebook').length > 0){
		// other input
		$(document).on('input','.form-control', function(){
			var input_val = $(this).val().trim(),
				pattern = /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))$/i,
		...
		$('.form-group input').on('mouseover',function(){
			$(this).parents('.form-group').removeClass('has-error');
		});

		$('#form-contacts-ebook .form__submit').click(function () {
			const form = $('#form-contacts-ebook');

			const rowData = new FormData(form[0]);
			...
			if($('.has-error').length > 0) check = false;
			if ($('#email_agree')[0].checked === false) {
				$('#email_agree').parents('.form-group').addClass('has-error');
				check = false;
			}
			if(check){
				// Create the new request
				...
				// Sends the request
				var send = xhr.send(JSON.stringify(data));
				var company = rowData.get('company') + ' | ' + $('.form').attr('data-ebook-name');
				rowData.set('company',company);

				handleFormSubmit('//traccoon.intellectsoft.net/forms/intellectsoft/insight-ebook', rowData, {
					type: 'ContactForm'
				}).then(res => {
					console.log(res);
				}).catch(error => {
